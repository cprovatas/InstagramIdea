...
- (void)awakeFromNib
{
    /* scroller won't appear in collection view w/o self.wantsLayer enabled, however
     the tableview scroller will appear either way, but the mouse over view
     doesnt appear on videos if self.wantsLayer IS called, very weird */
    if (![self.contentView.documentView isKindOfClass:[CustomTableView class]])
            self.wantsLayer = YES;
